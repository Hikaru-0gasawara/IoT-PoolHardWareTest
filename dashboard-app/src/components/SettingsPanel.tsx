...
export function SettingsPanel() {
  ...
  const lastMsgLabel = conn.lastMessageAt == null
    ? "—"
    : (now - conn.lastMessageAt < 10_000 ? "agora mesmo" : formatAge(now - conn.lastMessageAt));
  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
      ...
      <Card title="Conexão MQTT" icon={<Radio className="h-4 w-4" />}>
        ...
        {view.sublabel && (
          <div className="-mt-1 flex justify-end text-[10px]" style={{ color: toneToColor(view.tone) }}>
            {view.sublabel}
          </div>
        )}
        ...
        <div className="mt-3">
          <div className="text-[10px] uppercase tracking-wider text-aqua-text-muted">Tópicos publicados</div>
          <ul className="mt-1 space-y-0.5">
            {PUBLISHED_TOPICS.map((t) => <li key={t} className="font-tabular text-[11px] text-aqua-accent">{t}</li>)}
          </ul>
        </div>
      </Card>
      ...
      <Card title="Sistema" icon={<Wifi className="h-4 w-4" />}>
        ...
        <Row k="Faixa ideal ORP" v={`${THRESHOLDS.orp.idealMin} – ${THRESHOLDS.orp.idealMax} mV`} />
        <Row k="Faixa ideal condutividade" v={`${THRESHOLDS.condutividade.idealMin} – ${THRESHOLDS.condutividade.idealMax} µS/cm`} />
      ...
      <Card title="Equipe" icon={<Users className="h-4 w-4" />}>
        ...
        <div className="mt-3 space-y-1.5 text-xs">
          <div>
            <div className="text-[10px] uppercase tracking-wider text-aqua-text-muted">Grupo 1 — Controle de aquecimento</div>
            <div className="text-aqua-text">Martim Roxo · Vitor Yoshida</div>
          </div>
          <div>
            <div className="text-[10px] uppercase tracking-wider text-aqua-text-muted">Grupo 2 — Medição de água</div>
...
function Row({ k, v, valColor }: { k: string; v: string; valColor?: string }) {
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <span className="font-tabular text-right text-aqua-text" style={valColor ? { color: valColor } : undefined}>{v}</span>
    </div>
  );
}
