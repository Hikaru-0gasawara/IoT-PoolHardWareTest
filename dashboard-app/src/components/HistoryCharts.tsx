...
export function HistoryCharts() {
  ...
  const allPanelsOff =
    !panels.temperaturas && !panels.quimicos && !panels.alcalinidade && !panels.bomba;
  ...
  const pumpByHour = (() => {
    ...
    return Array.from(buckets.entries()).map(([hora, minutos]) => ({
      hora,
  ...
  const exportRows = data.map((d) => ({
    ...
    temp_piscina_c: d.temp_piscina,
    temp_coletor_c: d.temp_coletor,
  ...
  return (
    <div className="space-y-5">
          ...
          {(["24h", "7d", "30d"] as Range[]).map((r) => (
            <button
              ...
              className={`rounded-lg px-3.5 py-1.5 text-sm transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-aqua-accent ${
                range === r
                  ? "bg-aqua-accent text-aqua-bg font-medium"
      ...
      <div
        ...
        role="group"
        aria-label="Alternar painéis visíveis"
      >
      ...
      {visibleSeriesChips.length > 0 && (
        <div
          className="flex flex-wrap items-center gap-2"
          role="group"
          aria-label="Alternar séries dos gráficos"
        >
              ...
              <SummaryTable
                ...
                rows={[
                  { key: "temp_piscina", label: "Piscina", summary: summarize("temp_piscina") },
                  {
                    key: "temp_coletor",
                    label: "Coletor solar",
                    summary: summarize("temp_coletor"),
                  },
                ]}
              />
            }
            ...
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={data} margin={{ top: 10, right: 16, left: 0, bottom: 0 }}>
                ...
                {series.temp_piscina && (
                  <Line
                    type="monotone"
                    dataKey="temp_piscina"
                    name="Piscina"
                    stroke="var(--param-pool)"
                    strokeWidth={2.5}
                    dot={false}
                ...
                {series.temp_coletor && (
                  <Line
                    type="monotone"
                    dataKey="temp_coletor"
                    name="Coletor"
                    stroke="var(--param-solar)"
                    strokeWidth={2.5}
                    dot={false}
                    isAnimationActive={false}
                  />
                ...
                rows={[
                  {
                    key: "alcalinidade",
                    label: "Alcalinidade",
                    summary: summarize("alcalinidade"),
                  },
                ]}
              />
            }
          >
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={data} margin={{ top: 10, right: 16, left: 0, bottom: 0 }}>
                ...
                <Tooltip content={<TooltipBox />} />
                <ReferenceArea
                  y1={alcalinidadeT.idealMin}
                  y2={alcalinidadeT.idealMax}
                  fill="var(--status-ok)"
                  fillOpacity={0.1}
                />
                {series.alcalinidade && (
          ...
          <ChartCard
            ...
            fallback={
              <div className="sr-only">
                <p>
                  Período analisado: {rangeLabel}. Início:{" "}
                  {first ? new Date(first.t).toLocaleString("pt-BR") : "—"}. Fim:{" "}
                  {last ? new Date(last.t).toLocaleString("pt-BR") : "—"}. Total de minutos com
                  bomba ligada: {pumpMinutes}.
                </p>
                {pumpByHour.length > 0 && (
                  <table>
          ...
          >
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={pumpByHour} margin={{ top: 10, right: 16, left: 0, bottom: 0 }}>
                ...
                <Tooltip
                  contentStyle={{
                    backgroundColor: "var(--aqua-surface)",
          ...
          <tr key={key}>
            ...
            {summary ? (
              <>
                ...
                <td>
                  {fmt(summary.t.idealMin)}–{fmt(summary.t.idealMax)}
                  {summary.t.unit}
                </td>
