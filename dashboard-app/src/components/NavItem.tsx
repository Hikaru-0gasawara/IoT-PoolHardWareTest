...
export function NavItem({ to, label, active, testId }: NavItemProps) {
  ...
  const handleEnter = () => {
    ...
    animate(linkRef.current, { scale: 1.05, duration: MOTION.duration.fast, ease: MOTION.ease });
    if (!active && barRef.current) {
      animate(barRef.current, { scaleX: 0.6, opacity: 0.5, duration: MOTION.duration.fast, ease: MOTION.ease });
    }
  };

  const handleLeave = () => {
    if (linkRef.current) {
      animate(linkRef.current, { scale: 1, duration: MOTION.duration.fast, ease: MOTION.ease });
    }
    if (!active && barRef.current) {
      animate(barRef.current, { scaleX: 0, opacity: 0, duration: MOTION.duration.fast, ease: MOTION.ease });
    }
  };
