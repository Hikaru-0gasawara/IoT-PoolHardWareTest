...
export function SolarSystemHero({
  ...
  const flow =
    estado === "emergencia" ? "var(--status-crit)" : bombaOn ? "var(--flow)" : "var(--aqua-border)";
  ...
  const badgeStyle =
    estado === "emergencia"
      ? {
          ...
          background: "color-mix(in oklab, var(--status-crit) 12%, transparent)",
        }
      : estado === "circulando"
        ? {
            borderColor: "var(--flow)",
            color: "var(--flow)",
            background: "color-mix(in oklab, var(--flow) 12%, transparent)",
          }
        : {
            borderColor: "var(--aqua-border)",
    ...
    >
      <header className="mb-6 flex flex-wrap items-baseline justify-between gap-3">
        <div>
          <h2
            id="hero-heading"
            className="font-display text-2xl font-semibold tracking-tight text-aqua-text sm:text-3xl"
          >
            Aquecimento solar
          </h2>
          <p className="mt-1 text-sm text-aqua-text-muted">
            Piscina · Bomba · Coletor — visão integrada do circuito.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 items-center gap-4 lg:grid-cols-[1fr_1.4fr_1fr]">
        ...
        <figure className="flex flex-col items-center">
          <svg
            ...
            role="img"
            aria-label={`Piscina a ${fmtTemp(tempPiscina)}`}
          >
            <defs>
            ...
            <rect
              ...
              height="100"
              rx="12"
              fill="oklch(0.30 0.05 240)"
              stroke="var(--aqua-border)"
              strokeWidth="2"
            />
            <rect x="22" y="54" width="196" height="82" rx="6" fill="url(#ed-water)" />
            <path
              d="M 14 60 Q 30 54 46 60 T 78 60 T 110 60 T 142 60 T 174 60 T 206 60 T 238 60"
              fill="none"
        ...
        <div className="flex flex-col items-center">
          <svg
            viewBox="0 0 320 180"
            className="h-40 w-full max-w-[360px]"
            role="img"
            aria-label={bombaOn ? "Bomba ativa, água circulando" : "Bomba parada"}
          >
            <path
              d="M 8 56 H 312"
              stroke={flow}
            ...
            <g transform="translate(160 92)">
              <circle
                r="32"
                fill="var(--aqua-surface-2)"
                stroke={flow}
                strokeWidth="2"
                opacity={estado === "emergencia" ? 0.5 : 1}
              />
              <circle r="20" fill="var(--aqua-bg)" />
              <g
                stroke={bombaOn ? "var(--flow)" : "var(--aqua-text-muted)"}
                strokeWidth="2.2"
                strokeLinecap="round"
              >
          ...
          <div className="mt-2 text-center">
            ...
            >
              ΔT {deltaT >= 0 ? "+" : ""}
              {Number.isFinite(deltaT) ? deltaT.toFixed(1) : "—"}°
            </div>
        ...
        <figure className="flex flex-col items-center">
          <svg
            ...
            className="h-40 w-full max-w-[280px]"
            role="img"
            aria-label={`Coletor a ${fmtTemp(tempColetor)}`}
          >
            <defs>
